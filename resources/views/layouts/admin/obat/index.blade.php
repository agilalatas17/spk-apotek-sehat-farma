<x-admin-layout>
    ...
    <section class="px-4 sm:px-6 lg:px-12 mx-auto max-w-screen-xl">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-16">
            <div>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between mt-6 space-y-4 sm:space-y-0">
            ...
            @if ($data->onFirstPage())
                <span
                    class="inline-flex items-center px-4 py-2 text-sm text-white bg-gray-400 rounded-md cursor-not-allowed">
                    ← Previous
                </span>
            @else
                <a href="{{ $data->previousPageUrl() }}"
                    class="inline-flex items-center px-4 py-2 text-sm text-white bg-gray-600 rounded-md hover:bg-gray-500">
                    ← Previous
                </a>
            @endif
            ...
            <div class="flex items-center gap-x-2">
                ...
                {{ $data->currentPage() == $i ? 'text-white bg-gray-600' : 'text-gray-500 bg-gray-100 hover:bg-gray-300' }}">
                        {{ $i }}
                    </a>
                @endfor
            </div>
            ...
            @else
                <span
                    class="inline-flex items-center px-4 py-2 text-sm text-white bg-gray-300 rounded-md cursor-not-allowed">
                    Next →
                </span>
            @endif
        </div>
